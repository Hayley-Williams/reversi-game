...
socket.on('player_disconnected', function (payload) {
    ...
    /* If we are being notified that we we left the room, then ignore it */
    if (payload.socket_id == socket.id) {
        return;
    }
​
...
​
    ...
    console.log('checking payload player white socket '+payload.game.player_white.socket);
    console.log('checking payload player black socket '+payload.game.player_black.socket);
​
    /* Update my color */
                ...
                if (board[row][column] == ' ') {
                    ...
                    $('#' + row + '_' + column).click(function (r, c) {
                        return function () {
                            ...
                            console.log('*** Client Log Message: \'pay_token\' payload: ' + JSON.stringify(payload));
                            socket.emit('play_token', payload);
                        };
                    }(row, column));
                }
                else {
                    $('#' + row + '_' + column).remove('hovered_over');
                }
            }
        }
    }
